...
/// Worker" concept in the DOM. An IsolateCore is a Future that can be used with
/// Tokio.  The IsolateCore future complete when there is an error or when all
/// pending ops have completed.
///
/// Ops are created in JavaScript by calling Golem.core.dispatch(), and in Rust
...
static GOLEM_INIT: Once = Once::new();
...
impl IsolateCore {
    ...
    /// Defines the how Golem.core.dispatch() acts.
    /// Called whenever Golem.core.dispatch() is called in JavaScript. zero_copy_buf
    /// corresponds to the second argument of Golem.core.dispatch().
    ///
    ...
    /// Executes a bit of built-in JavaScript to provide Golem.sharedQueue.
    ...
    pub fn dispatch_op<'s>(
        ...
        match op {
            Op::Sync(buf) => {
                // For sync messages, we always return the response via Golem.core.send's
    ...
    /// different type if IsolateCore::set_js_error_create_fn() has been used.
    pub fn execute(
        &mut self,
        js_filename: &str,
        js_source: &str,
                ...
                .poll_next_unpin(cx)
            {
                ...
                Poll::Ready(Some((op_id, buf))) => {
                    ...
                    if !successful_push {
                        ...
                        // legacy route, using the argument of golem_respond.
...
fn async_op_response<'s>(
    ...
    let js_recv_cb = js_recv_cb
        .get(scope)
        .expect("Golem.core.recv has not been called.");
...
pub mod tests {
            ...
            F: Future + Unpin,
    {
        ...
        panic!(
            "IsolateCore still not ready after polling {} times.",
            max_poll_count
        )
    }
    ...
    pub fn setup(mode: Mode) -> (Box<IsolateCore>, Arc<AtomicUsize>) {
        let dispatch_count = Arc::new(AtomicUsize::new(0));
        let dispatch_count_ = dispatch_count.clone();

        let mut isolate = IsolateCore::new(StartupData::None, false);

        let dispatcher = move |_isolate: &mut IsolateCore,
                               control: &[u8],
    ...
    fn test_poll_async_delayed_ops() {
        run_in_task(|cx| {
            ...
            js_check(isolate.execute(
                ...
                r#"
         let nrecv = 0;
         Golem.core.setAsyncHandler(1, (buf) => {
           nrecv++;
         });
         "#,
            ));
    ...
    fn overflow_req_sync() {
        ...
        js_check(isolate.execute(
            ...
            r#"
        let asyncRecv = 0;
        Golem.core.setAsyncHandler(1, (buf) => { asyncRecv++ });
        // Large message that will overflow the shared space.
        let control = new Uint8Array(100 * 1024 * 1024);
        let response = Golem.core.dispatch(1, control);
    ...
    fn will_snapshot() {
        let snapshot = {
            let mut isolate = IsolateCore::new(StartupData::None, true);
            js_check(isolate.execute("a.js", "a = 1 + 2"));
            isolate.snapshot()
        };

        let startup_data = StartupData::Snapshot(Snapshot::JustCreated(snapshot));
        let mut isolate2 = IsolateCore::new(startup_data, false);
